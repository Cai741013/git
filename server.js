...
const proposalSchema = {
  ...
  required: ['researchTrends','rows','assumptions','products','flow','metrics','values','risk','conclusion','ai','phases','benefit','revisionSummary','sources'],
  properties: {
    ...
    risk: { type:'string' }, conclusion: { type:'string' }, ai: { type:'string' }, benefit: { type:'string' }, revisionSummary: { type:'string' },
...
function buildPrompt(customer){
  ...
  const iterationContext=customer.feedback && customer.aiScenario
    ? `\n这是需要修改的当前方案（JSON）：\n${JSON.stringify(customer.aiScenario)}\n\n本轮用户修改意见：${customer.feedback}\n当前版本：第 ${Number(customer.version)||2} 版\n\n迭代要求：\n1. 必须直接修改与反馈相关的 rows、products、flow、metrics、values、risk、conclusion、ai、phases 或 benefit 字段，不能只是换一种说法。\n2. 与反馈无关且正确的内容可以保留；不得破坏客户事实边界。\n3. revisionSummary 用一句话具体说明本轮实际改动了哪些内容，禁止只写“已优化方案”。\n`
    : '\n这是首次生成。revisionSummary 返回“初版方案生成”。\n';
  return `角色：你是一位拥有5年企业数字化转型经验的飞书商业化售前顾问。\n\n目标：针对以下客户生成精准、可落地的飞书 AI 解决方案。\n客户名称：${customer.clientName}\n行业：${customer.industry}\n规模：${customer.scale}\n客户原始痛点：${customer.painPoints}\n已拆分的客户事实（必须按此顺序逐条诊断，不得增加）：\n${painFacts.map((x,i)=>`${i+1}. ${x}`).join('\n')}\n${iterationContext}\n飞书产品知识库：${productKnowledge}\n\n事实边界（必须严格遵守）：\n1. rows 只能包含上面的“客户事实”，数量和顺序必须完全一致；rows 第一列直接使用对应客户事实，不得新增“客户管理粗放”等客户未提及的问题。\n2. 根因是顾问诊断，不得写成已经证实的客户事实；信息不足时写“具体原因待需求访谈确认”。\n3. 可能有价值但客户未提供依据的判断，只能写入 assumptions，并明确为待确认假设；不得放入 rows、risk 或“当前状态”。没有则返回空数组。\n4. risk、conclusion 和 values 的“当前状态”只能引用客户输入；预测数字必须明确写“目标值”或“预计”，不得编造已发生数据。\n\n方案标准：\n1. 每个根因具体到岗位、数据、流程或责任机制，禁止用“信息分散、流程人工”等空话替代客户场景。\n2. 产品组合必须具体到上述飞书模块，明确如何操作、由谁使用。\n3. 数字必须基于客户输入或标注为“目标/预计”。\n4. 给出三期实施路径：1-2周试点、1个月推广、3个月智能化。\n5. 如使用网络搜索，只引用可靠来源；找不到可靠数据时明确使用合理目标值，不伪造案例。\n6. 输出简体中文，短句、专业、有咨询感。\n\n只返回一个 JSON 对象，不要输出 Markdown 代码围栏或其他文字。字段必须为：researchTrends（三条字符串）、rows（一至五个[客户事实,顾问根因诊断,影响范围]）、assumptions（零至三个待确认假设字符串）、products（三至六个[飞书产品,具体用法]）、flow（四至六步）、metrics（三个[目标值,指标名]）、values（三至五个[指标,当前状态,预期改善,量化价值]）、risk、conclusion、ai、phases（三期字符串）、benefit、revisionSummary（本轮具体修改摘要）、sources（来源对象数组，无实时来源时返回空数组）。`;
}
...
function parseJson(text){
  ...
  for(const key of ['risk','conclusion','ai','benefit','revisionSummary']) if(typeof parsed[key]!=='string') throw new Error(`模型结果缺少文本字段：${key}`);
